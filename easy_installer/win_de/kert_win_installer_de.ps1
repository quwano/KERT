...
if ($null -eq $pyExe) {
    ...
    Write-Host "[1/4] textgrid wird installiert..."
    ...
    Write-Host "[2/4] saxonche wird installiert..."
    ...
    if ($arch -eq "ARM64") {
        ...
        Write-Host "  2. Pruefen, ob die AMD64-Version in der Eingabeaufforderung angezeigt wird:"
        Write-Host "     py -0p"
        Write-Host "     -> Eine Python-3.12-Zeile OHNE 'arm64' im Pfad sollte erscheinen"
        Write-Host "  3. Alle Pakete installieren:"
        Write-Host "     py -3.12 -m pip install textgrid saxonche pdfplumber Pillow"
        Write-Host "  Hinweis: Schritte 1-3 nur einmalig. Danach KERT einfach per start_kert.bat starten."
        Write-Host ""
    ...
    Write-Host "[3/4] pdfplumber / Pillow wird installiert..."
    ...
    Write-Host "[4/4] tkinterdnd2 wird installiert..."
    & $pyArgs[0] @($pyArgs[1..99] + @("-m", "pip", "install", "tkinterdnd2>=0.4.2,<0.5.0"))
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[Fertig] tkinterdnd2 wurde erfolgreich installiert."
    } else {
        Write-Host "[Fehler] tkinterdnd2-Installation fehlgeschlagen." -ForegroundColor Red
    }
}
