...
if ($null -eq $pyExe) {
    ...
    if ($arch -eq "ARM64") {
        ...
        Write-Host "  2. コマンドプロンプトで以下を実行して AMD64 版が表示されることを確認:"
        Write-Host "     py -0p"
        Write-Host "     → パスに 'arm64' を含まない Python 3.12 の行があればOK"
        Write-Host "  3. 全パッケージをインストール:"
        Write-Host "     py -3.12 -m pip install textgrid saxonche pdfplumber Pillow"
        Write-Host "  ※ 手順 1〜3 は初回のみ。以降は KERT フォルダ内の start_kert.bat で起動できます。"
        Write-Host ""
