...
if ($null -eq $pyExe) {
    ...
    if ($arch -eq "ARM64") {
        ...
        Write-Host "  2. Run the following in Command Prompt to verify the AMD64 version is listed:"
        Write-Host "     py -0p"
        Write-Host "     -> A Python 3.12 entry whose path does NOT contain 'arm64' should appear"
        Write-Host "  3. Install all packages:"
        Write-Host "     py -3.12 -m pip install textgrid saxonche pdfplumber Pillow"
        Write-Host "  Note: Steps 1-3 are one-time setup. After that, launch KERT via start_kert.bat."
        Write-Host ""
