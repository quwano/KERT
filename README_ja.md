...
入力形式として **CommonMark 拡張記法**（`.txt` / `.md`）、**XML 形式**（`.xml`）、**PDF 形式**（`.pdf`）に対応しています。
...
PDF 入力 (.pdf)
    ↓  pdfplumber: テキスト抽出・見出し検出（3種のモードから選択）
    ↓  Pillow: 図版抽出 → intermediate_products/figures/
    ↓  TTS → WAV → MP3 → TextGrid（MFA）→ EPUB パッケージング
EPUB3 ファイル（音声同期付き）

PDF → Markdown 出力（音声なし）:
    ↓  pdfplumber: テキスト抽出
Markdown ファイル（.md）← yomikae・ルビ等を追記後、CommonMark として再処理可能
```
...
**Windows**: KERT フォルダ内の `start_kert.bat` をダブルクリックして起動します。

**macOS**: KERT フォルダ内の `start_kert.command` をダブルクリックして起動します。

起動すると以下のウィンドウが表示されます。

![KERT GUIスクリーンショット](docs/images/GUI.png)

以下の順に設定し、「実行 / Run」ボタンを押してください。

1. **言語**: 日本語 / English (US) / Deutsch から選択
2. **入力形式**: CommonMark (.txt/.md) / XML / PDF から選択
3. **処理モード**: 単一ファイル / フォルダ / PDF → MD 変換のみ から選択
4. **（PDF のみ）見出し検出**: レイアウト重視 / 法令文言 / 番号記号 から選択
5. **パス**: 入力ファイルまたはフォルダのパスを入力（「参照... / Browse...」ボタンまたはウィンドウへのドラッグ＆ドロップも可）
6. **中間ファイル**: 「中間ファイルを残す」をチェックすると処理後も中間ファイルが保持されます

処理中はウィンドウ下部のログエリアに進捗が表示されます。完了後にダイアログが表示されます。

### 終了方法

処理完了後にウィンドウを閉じてください（タイトルバーの閉じるボタン、または macOS では Cmd+Q）。

> **注意**: 処理中にウィンドウを閉じると、処理は即座に中断されます。生成途中の EPUB ファイルが残る場合があります。

コマンドラインから直接実行することもできます（上級者向け）:
...
KERT は **CommonMark 拡張記法**、**XML 形式**、**PDF 形式** の 3 種類の入力形式に対応しています。いずれの形式でも、[書誌情報ファイル（metadata.txt）](#書誌情報ファイルmetadatatxt)の添付が必須です。
...
| `$数式$` | インライン数式（TeX） | `$x^2 + y^2$` | インライン MathML |
| `$$数式$$` | ブロック数式（TeX） | `$$\int_0^1 x\,dx$$` | ブロック MathML（中央揃え） |
...
GFM（GitHub Flavored Markdown）パイプテーブル記法で表を記述します。
...
- 1行目がヘッダー行（`<th>`）、3行目以降がデータ行（`<td>`）になります。
- 2行目（`---`）は区切り行で、列数を定義します。
- **セル単位でハイライト**されます。ヘッダーセル（`<th>`）も読み上げ対象です。
- 各セルには書式記法（ルビ、読み替え、太字など）を使用できます。
...
<table>
  ...
  <tr><td>データA</td><td>データB</td></tr>
  <tr><td>データD</td><td>データE</td></tr>
</table>
```

- `<tr>` は行、`<th>` は見出しセル、`<td>` はデータセルです。
- **セル単位でハイライト**されます。`<th>` も読み上げ対象です。
- 各セルには装飾要素（`<ruby>`、`<u>` など）を使用できます。
...
KERT は PDF ファイルからテキストと図版を抽出し、EPUB または Markdown を生成します。

> **注意**: テキストが埋め込まれた PDF のみ対応しています。スキャンした画像のみの PDF は非対応です。

#### 見出し検出モード

PDF 処理時に、以下の3種類の見出し検出モードから選択します：

| モード | 適した文書 | 検出方法 |
|--------|---------|---------|
| **1. レイアウト重視** | ウェブページ・デザイン文書等 | フォントサイズ・色・背景矩形 |
| **2. 法令文言** | 法令・規則・契約書等 | 第n章、第n条 等のパターン |
| **3. 番号付き・記号付き** | 報告書・マニュアル等 | ■、【...】、`1.`、`①`、`⑴` |

#### 見出しパターン設定ファイル

`resources/pdf_heading_patterns.json` に各モードの正規表現パターンが記載されています。このファイルを直接編集してパターンを追加・削除できます。

#### 図版の抽出

PDF に埋め込まれた画像は自動的に `intermediate_products/figures/` に抽出され、生成した EPUB に組み込まれます。Pillow（`pip install Pillow`）が必要です。

#### PDF → Markdown 出力

音声や EPUB を生成せず、PDF を Markdown ファイル（`.md`）として出力するモードもあります。生成した Markdown に yomikae・ルビ等を追記した後、CommonMark 入力として再処理できます。
...
> **簡単インストールを使用した場合**: `easy_installer/` のインストーラスクリプトでセットアップした場合、このセクションの手順は**数式サポートを除いてすべて実施済み**です。数式サポート（オプション）のみ必要に応じて追加してください。
...
| `pdfplumber` | PDF テキスト・画像抽出 |
| `Pillow` | PDF 図版画像の保存 |

saxonche は XML 入力時の変換に使用します。CommonMark または PDF 入力のみの場合は不要です。
pdfplumber と Pillow は PDF 入力に必要です。Pillow は PDF 内の図版を抽出する場合にのみ必要です。

### 数式サポート（オプション）

CommonMarkファイル内の TeX 記法（`$...$`、`$$...$$`）および XML ファイル内の MathML（`<math>`）による数式入力に対応しています。この機能を使用するには、以下のツールが必要です。

| ツール | 用途 | インストール方法 |
|-------|------|--------------|
| [pandoc](https://pandoc.org/) | TeX → MathML 変換 | `brew install pandoc` / [pandoc.org](https://pandoc.org/installing.html) |
| [Node.js](https://nodejs.org/) | speech-rule-engine の実行環境 | `brew install node` / [nodejs.org](https://nodejs.org/) |
| [speech-rule-engine](https://github.com/zorkow/speech-rule-engine) | MathML → 読み上げテキスト変換 | 下記参照 |

#### 数式サポートの対応言語

数式の **EPUB への表示**（MathML）はすべての言語で動作します。ただし、数式の **音声読み上げ**（再生時の読み上げ）には、speech-rule-engine と MFA の両方が対象言語に対応している必要があります。

speech-rule-engine の対応言語については [speechruleengine.org](https://speechruleengine.org/) を参照してください。

| KERT 言語 | 数式表示 | 数式読み上げ |
|-----------|---------|------------|
| 日本語（ja_JP） | ✓ | ✗ 非対応（2026年3月現在、SRE が日本語に非対応） |
| 英語（en_US）   | ✓ | ✓ |
| ドイツ語（de_DE）| ✓ | ✓ |

日本語入力の場合、数式は EPUB に MathML として表示されますが、正常な音声読み上げは行われません。数式以外のテキストは通常どおり処理されます。

#### speech-rule-engine のインストール

speech-rule-engine は **KERT プロジェクトルートに** ローカルインストールする必要があります。

> **重要**: `npm install` は必ず **KERT プロジェクトのルートディレクトリで** 実行してください。別のディレクトリで実行しても npm はエラーを出しませんが、KERT はツールを認識できません。

```bash
cd /path/to/KERT   # KERT プロジェクトルートに移動
npm install speech-rule-engine
```

入力ファイルに数式が検出された際にこれらのツールが見つからない場合、KERTは警告を表示し、処理を中止するか数式なしで続行するかを選択できます。
...
### 読みマップ設定（`resources/reading_map.json`）

TTS・MFA の音声アライメントで、特殊文字をどのように読み上げるかを定義します。組み込みのマップでは丸数字（①②…）、ローマ数字（Ⅰ Ⅱ…）、括弧類を定義しています。`resources/reading_map.json` を直接編集して追加・上書きできます。
...
`""` に設定すると読み上げテキストから除去されます（MFA が認識できない文字の除去に有効）。文字列に設定すると組み込みの読みを上書きします。

### PDF 見出しパターン設定（`resources/pdf_heading_patterns.json`）

PDF の見出し検出モードごとの正規表現パターンを定義します。文書の種類に合わせてパターンを追加・削除して調整できます。レイアウトモードのスコア閾値も設定可能です。
